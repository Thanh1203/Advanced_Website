    ...
    <div class="showtime_ctn_select-movie row g-3">
        <div class="select_movie-lists">
            <select class="form-select" v-model="selectMovie">
              ...
              <option :value="`${item.id}`" v-for="item in products" :key="item.id">{{ item.name }}</option>
            </select>
        </div>
        <div class="select_movie-btn">
            <button type="button" class="btn btn-primary" @click.prevent="getMoiveShowTime">Get movie</button>
        </div>
    </div>
        ...
        <div class="showtime_ctn_select-movie row g-3">
            <div class="select_movie-lists">
                <select class="form-select" v-model="selectShowtime">
                  ...
                  <option :value="`${item}`" v-for="item in calender" :key="item">{{ item }}</option>
                </select>
            </div>
            <div class="select_movie-btn">
                <button type="button" class="btn btn-primary" @click.prevent="getShowTime">Get showtime</button>
            </div>
        </div>
        <div class="showtime_table-ctn" >
            <table class="showtime_table">
                ...
                <tbody class="body_table" v-show="isTable">
                    <tr v-for="(item,index) in showtime" :key="item.index" class="body_table-row">
                        <td>{{ item.moviedate }}</td>
                        <td>
                            <span v-show="datachangeIndex != index">{{ item.timeslot }}</span>
                            <input type="time" v-model="datachange.timeslot" v-show="datachangeIndex == index">
                        </td>
                        <td>
                            <span v-show="datachangeIndex != index">{{ item.nofTickets }}</span>
                            <input type="number" v-model="datachange.nofTickets" min="0" v-show="datachangeIndex == index">
                        </td>
                        <td class="table_option-body">
                            <font-awesome-icon :icon="['fas', 'pen-to-square']" size="xl" v-show="datachangeIndex != index" @click="editShowtime(index)" />
                            <button type="button" class="btn btn-success"  v-show="datachangeIndex == index" @click.prevent="saveChange(item.row_number)">Save</button>
                        </td>
                        <td class="table_option-body">
                            <font-awesome-icon :icon="['fas', 'trash-can']" size="xl" v-show="datachangeIndex != index" @click="deleteShowTime(item.row_number)" />
                            <button type="button" class="btn btn-danger" style="margin: 0 auto;" v-show="datachangeIndex == index" @click.prevent="cancelChange">Cancel</button>
                        </td>
                    </tr>
                </tbody>
            </table>
            <div class="showtime_btn" v-show="isTable">
                <button type="button" class="btn btn-primary" @click.prevent="addNewShowtime">Add showtimes</button>
            </div>
        </div>
    </div>
...
let showtime = ref([])
const calender = ref(["Thứ 2", "Thứ 3", "Thứ 5", "Thứ 6", "Thứ 7", "Chủ nhật"])
...
function getMoiveShowTime() {
    if (selectMovie.value != 'default' && selectMovie.value != tempSelectMovie) {
        ...
        productSeleted.value = products.value.find(item => item.id == selectMovie.value)
        setTimeout(() => {
            scrollToBottom()
        }, 1);
...
async function getShowTime() {
    if (selectShowtime.value != 'default') {
        await axios.get(`${showtimeCoureApi}/${selectMovie.value}/${selectShowtime.value}`).then(data => {
...
async function saveChange(rownumber) {
    ...
    await axios.patch(`${showtimeCoureApi}/${selectMovie.value}/${rownumber}`, formData, {
        headers: {
...
async function deleteShowTime(index) {
    try {
        if (selectMovie.value != null) {
            await axios.delete(`${showtimeCoureApi}/${selectMovie.value}/${index}`).then(() => {
                getShowTime()
            })
        }
    } catch (error) {
