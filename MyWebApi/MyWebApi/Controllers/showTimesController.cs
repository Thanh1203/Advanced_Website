...
namespace MyWebApi.Controllers
{
    ...
    public class ShowTimesController : ControllerBase
    {
        ...
        {
            var stimes = await _context.ShowTimes.Where(item => item.IdMovieShowtime == id).ToListAsync();
            return Ok(stimes);
        }
        ...
        public IActionResult createNewShowTime(ShowTimes model)
        {
            try
            {
                ...
                {
                    IdMovieShowtime = model.IdMovieShowtime,
        ...
        {
            var stimes = _context.ShowTimes.SingleOrDefault(item => item.IdMovieShowtime == id);
            if (stimes != null)
            {
